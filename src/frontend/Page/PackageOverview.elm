...
viewTypeAnnotation entry =
  let
    -- No links on diff view.
    -- We might be able to replace the "isNormal" for a
    --  well structured docContext ?
    docsContext =
      { current = ""
      , available = Set.empty
      }
    ...
    kids =
      Entry.viewTypeAnnotation False Dict.empty docsContext entry
